...
def mission_scenario() -> dict:
    ...
    X0 = sample_mc_initial_states(x0, _p0_mission(), particles, rng, True)
    return {
        "propagator": prop_mc,
        "det_propagator": prop_det,
        "t_grid": t_grid,
        "env": env,
        "X0": X0,
    }
...
def attitude_scenario() -> dict:
    ...
    particles = int(scenario.get("particles", 32))
    rng = np.random.default_rng(seed)
    X0 = sample_mc_initial_states(x0, _p0_attitude_for_scenario(scenario), particles, rng, False)
    return {
        "propagator": prop_mc,
        "det_propagator": prop_det,
        "t_grid": t_grid,
